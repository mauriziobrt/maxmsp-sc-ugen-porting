...
// method prototypes
void *bhishelf_new(t_symbol *s, long argc, t_atom *argv);
void bhishelf_free(t_bhishelf *x);
void bhishelf_assist(t_bhishelf *x, void *b, long m, long a, char *s);
//void bhishelf_float(t_bhishelf *x, double f);
void bhishelf_dsp64(t_bhishelf *x, t_object *dsp64, short *count, double samplerate, long maxvectorsize, long flags);
void bhishelf_perform64(t_bhishelf *x, t_object *dsp64, double **ins, long numins, double **outs, long numouts, long sampleframes, long flags, void *userparam);


// global class pointer variable
static t_class *bhishelf_class = NULL;
...
void ext_main(void *r)
{
    ...
    t_class *c = class_new("bhishelf~", (method)bhishelf_new, (method)dsp_free, (long)sizeof(t_bhishelf), 0L, A_GIMME, 0);

//    class_addmethod(c, (method)bhishelf_float,        "float",    A_FLOAT, 0);
    class_addmethod(c, (method)bhishelf_dsp64,        "dsp64",    A_CANT, 0);
    class_addmethod(c, (method)bhishelf_assist,    "assist",    A_CANT, 0);
...
//        x->m_a0 = a * ((a + 1.) + j + k) * b0rz;
//        x->m_a1 = -2. * a * ((a - 1.) + i) * b0rz;
//        x->m_a2 = a * ((a + 1.) + j - k) * b0rz;
//        x->m_b1 = -2. * ((a - 1.) - i) * b0rz;
//        x->m_b2 = ((a + 1.) - j - k) * -b0rz;
...
//    lowshelf_updateCoefficients(x, x->m_freq, x->m_rs, x->m_db);
    ...
    object_method(dsp64, gensym("dsp_add64"), x, bhishelf_perform64, 0, NULL);
}
